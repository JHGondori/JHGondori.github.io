---
title: The Basic Counting Arguments
...
이 post에서는 아 이런느낌이구나~ 정도 알아가시면 될 거에요.
...
이제 $i$번째 $k$ - clique가 단색인 사건을 $A_i$라고 합시다. 이 때 사건 $A_i$가 일어나려면 간선 하나의 색을 아무거나 하나로 두면, 나머지 $\binom{k}{2}-1$ 개의 간선은 그 색으로 결정되어야 하므로 $A_i$가 일어날 확률은 아래와 같다는 사실을 알 수 있습니다. $$\Pr(A_i) = 2^{-\binom{k}{2}+1}$$ 
이제 여기서 Union bound를 쓰면, 적어도 하나가 단색일 확률에 대한 식이 아래와 같이 나오게 됩니다. $$\Pr\left(\bigcup_{i=1}^{\begin{pmatrix}n\\k\end{pmatrix}}A_i\right)\leq\sum_{i=1}^{\begin{pmatrix}n\\k\end{pmatrix}}\Pr(A_i)=\begin{pmatrix}n\\k\end{pmatrix}2^{-\begin{pmatrix}k\\2\end{pmatrix}+1}$$
그렇습니다. Theorem에 있던 뭔 소린지 모르겠던 If 조건이 튀어나온 것을 볼 수 있습니다!
~~너무 억지 아니여?~~ 
자 여기서 만약 $\binom{n}{k} 2^{-\binom{k}{2}+1} \lt 1$이라면, $$\Pr\left(\bigcap_{i=1}^{\begin{pmatrix}n\\k\end{pmatrix}}\bar{A_i}\right) = 1 - \Pr\left(\bigcup_{i=1}^{\begin{pmatrix}n\\k\end{pmatrix}}A_i\right) \gt 0$$
...
이렇게 Probabilistic method를 적용하는 하나의 예시를 알아보았습니다. 다음부터 유용한 기법(?)들을 알아가봅시다. :)
